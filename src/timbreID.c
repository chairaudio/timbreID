...
static void timbreID_writeText(t_timbreID *x, t_symbol *s)
{
    ...
    {    
		...
		while(j<=x->x_attributeHi)
		{
			...
			if(thisAttribute>=x->x_instances[thisAttribute].length)
			{	
...
//			What's the best float resolution to print?	
			fprintf(filePtr, "%0.20f ", thisFeatureData);
			j++;
		};
...
static void timbreID_readText(t_timbreID *x, t_symbol *s)
{
	...
	t_float *featurePtr;
    char *fileName, fileNameBuf[MAXPDSTRING], textLine[MAXPDSTRING];
	...
	numInstances = 0;
	while(fgets(textLine, MAXPDSTRING, filePtr))
		numInstances++;
...
static void timbreID_MATLAB(t_timbreID *x, t_symbol *file_symbol, t_symbol *var_symbol)
{
    ...
    t_float *featurePtr;
    char fileNameBuf[MAXPDSTRING], *fileName, *varName;
...
static void timbreID_OCTAVE(t_timbreID *x, t_symbol *file_symbol, t_symbol *var_symbol)
{
	...
	fprintf(filePtr, "# Created by timbreID version 0.7\n");
	fprintf(filePtr, "# name: %s\n", varName);
	fprintf(filePtr, "# type: matrix\n");
	fprintf(filePtr, "# rows: %i\n", x->x_numInstances);
	fprintf(filePtr, "# columns: %i\n", x->x_maxFeatureLength);
