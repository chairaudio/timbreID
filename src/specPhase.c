...
static t_class *specPhase_class;

typedef struct _specPhase
{
    t_object x_obj;
...
/* ------------------------ specPhase -------------------------------- */
static void specPhase_resizeWindow(t_specPhase *x, t_sampIdx oldWindow, t_sampIdx window, t_sampIdx startSamp, t_sampIdx *endSamp)
{			
...
static void specPhase_analyze(t_specPhase *x, t_floatarg start, t_floatarg n)
{
	t_garray *a;
	...
	else
	{
		...
		if(x->x_window != window)
			specPhase_resizeWindow(x, x->x_window, window, startSamp, &endSamp);
...
static void specPhase_chain_fftData(t_specPhase *x, t_symbol *s, int argc, t_atom *argv)
{
...
static void specPhase_bang(t_specPhase *x)
{
	t_garray *a;
	...
	else
	{
		...
		specPhase_analyze(x, startSamp, window);
	}
}


static void specPhase_set(t_specPhase *x, t_symbol *s)
{
	t_garray *a;
...
static void specPhase_print(t_specPhase *x)
{
...
static void specPhase_samplerate(t_specPhase *x, t_floatarg sr)
{
	if(sr<MINSAMPLERATE)
...
static void specPhase_window(t_specPhase *x, t_floatarg w)
{
	t_sampIdx endSamp;
    ...
    specPhase_resizeWindow(x, x->x_window, w, 0, &endSamp);
}


static void specPhase_windowFunction(t_specPhase *x, t_floatarg f)
{
    f = (f<0)?0:f;
    f = (f>4)?4:f;
...
static void *specPhase_new(t_symbol *s, int argc, t_atom *argv)
{
    t_specPhase *x = (t_specPhase *)pd_new(specPhase_class);
	t_sampIdx i;
//	t_garray *a;
...
static void specPhase_free(t_specPhase *x)
{
	// free FFTW stuff
...
void specPhase_setup(void)
{
    ...
    class_new(
    	gensym("specPhase"),
    	(t_newmethod)specPhase_new,
    	(t_method)specPhase_free,
        sizeof(t_specPhase),
        CLASS_DEFAULT,
        A_GIMME,
		0
	...
	class_addcreator(
		(t_newmethod)specPhase_new,
		gensym("timbreIDLib/specPhase"),
		A_GIMME,
		0
	);

	class_addbang(specPhase_class, specPhase_bang);

	class_addmethod(
		specPhase_class,
        (t_method)specPhase_analyze,
		gensym("analyze"),
        A_DEFFLOAT,
        A_DEFFLOAT,
		0
	...
	class_addmethod(
		specPhase_class,
		(t_method)specPhase_chain_fftData,
	...
	class_addmethod(
		specPhase_class,
        (t_method)specPhase_windowFunction,
