...
static t_class *specPhase_tilde_class;

typedef struct _specPhase_tilde
{
    t_object x_obj;
...
static void specPhase_tilde_bang(t_specPhase_tilde *x)
{
...
static void specPhase_tilde_print(t_specPhase_tilde *x)
{
...
static void specPhase_tilde_window(t_specPhase_tilde *x, t_floatarg w)
{
...
static void specPhase_tilde_overlap(t_specPhase_tilde *x, t_floatarg o)
{
...
static void specPhase_tilde_windowFunction(t_specPhase_tilde *x, t_floatarg f)
{
    f = (f<0.0)?0.0:f;
    f = (f>4.0)?4.0:f;
...
static void *specPhase_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
    t_specPhase_tilde *x = (t_specPhase_tilde *)pd_new(specPhase_tilde_class);
	t_sampIdx i;
...
static t_int *specPhase_tilde_perform(t_int *w)
{
    t_uShortInt n;
	t_sampIdx i;
	
	t_specPhase_tilde *x = (t_specPhase_tilde *)(w[1]);
...
static void specPhase_tilde_dsp(t_specPhase_tilde *x, t_signal **sp)
{
	dsp_add(
		specPhase_tilde_perform,
		3,
		x,
		sp[0]->s_vec,
		sp[0]->s_n
...
static void specPhase_tilde_free(t_specPhase_tilde *x)
{
...
{
    specPhase_tilde_class = 
    class_new(
    	gensym("specPhase~"),
    	(t_newmethod)specPhase_tilde_new,
    	(t_method)specPhase_tilde_free,
        sizeof(t_specPhase_tilde),
        CLASS_DEFAULT, 
        A_GIMME,
		0
    );
    ...
    CLASS_MAINSIGNALIN(specPhase_tilde_class, t_specPhase_tilde, x_f);

	class_addbang(specPhase_tilde_class, specPhase_tilde_bang);

	class_addmethod(
		specPhase_tilde_class, 
        (t_method)specPhase_tilde_print,
		gensym("print"),
		0
	);
	...
	class_addmethod(
		specPhase_tilde_class, 
        (t_method)specPhase_tilde_window,
		gensym("window"),
		A_DEFFLOAT,
		0
	);
	...
	class_addmethod(
		specPhase_tilde_class, 
        (t_method)specPhase_tilde_overlap,
		gensym("overlap"),
		A_DEFFLOAT,
		0
	);
	...
	class_addmethod(
		specPhase_tilde_class,
        (t_method)specPhase_tilde_windowFunction,
